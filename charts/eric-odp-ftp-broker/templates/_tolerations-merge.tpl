...
*/}}
{{- define "eric-odp-ftp-broker.merge-tolerations" -}}
  {{- if (.root.Values.global).tolerations }}
      ...
      {{- range $globalItem := $globalTolerations -}}
        {{- $globalItemId := include "eric-odp-ftp-broker.merge-tolerations.get-identifier" $globalItem -}}
        {{- range $serviceItem := $serviceTolerations -}}
          {{- $serviceItemId := include "eric-odp-ftp-brokere.merge-tolerations.get-identifier" $serviceItem -}}
      ...
      {{- range $globalItem := $globalTolerations -}}
        ...
        {{- range $matchItem := $matchingItems -}}
          {{- $matchItemId := include "eric-odp-ftp-broker.merge-tolerations.get-identifier" $matchItem -}}
      ...
      {{- end -}}
      {{- range $serviceItem := $serviceTolerations -}}
        {{- $serviceItemId := include "eric-odp-ftp-broker.merge-tolerations.get-identifier" $serviceItem -}}
        {{- $matchCount := 0 -}}
        {{- range $matchItem := $matchingItems -}}
          {{- $matchItemId := include "eric-odp-ftp-broker.merge-tolerations.get-identifier" $matchItem -}}
...
*/}}
{{ define "eric-odp-ftp-broker.merge-tolerations.get-identifier" }}
  {{- $keyValues := list -}}
